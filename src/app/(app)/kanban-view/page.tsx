...
const Task: React.FC<{task: TaskType; column: string; moveTask: (id: string, column: string) => void}> = ({task, column, moveTask}) => {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className="bg-white p-4 mb-2 rounded-lg shadow hover:shadow-lg transition-all">
...
const KanbanBoard = () => {
  ...
  const moveTask = async (taskId: string, destinationColumn: string) => {
        ...
        <DndProvider backend={HTML5Backend}>
          <div id="kanban-card" className="flex lg:flex-row sm:flex-col xs:flex-col xs:max-w-full mt-10 gap-4 mx-10">
            {["To-Do", "In-Progress", "Completed"].map((column) => (
              <Column key={column} column={column} tasks={tasks[column]} moveTask={moveTask} />
            ))}
          </div>
        </DndProvider>
      </div>
